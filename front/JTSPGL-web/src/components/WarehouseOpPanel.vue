...
<template>
    <div class="warehouse-op-panel">
      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          <input v-model="queryInput" placeholder="输入商品ID" class="form-control mb-2" />
      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          <input v-model="replenishProduct" placeholder="商品ID" class="form-control mb-2" />
          <input v-model.number="replenishQty" placeholder="补货数量" class="form-control mb-2" />
      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          ...
          <input v-model.number="transferQty" placeholder="调货数量" class="form-control mb-2" />
          <select v-model="selectedWarehouse" class="form-select mb-2">
            <option disabled value="">选择目标仓库</option>
            <option v-for="w in warehouseList" :key="w.id" :value="w.id">{{ w.name }}</option>
          </select>
          <button class="btn btn-warning" @click="transfer">提交调货</button>
        </div>
      </div>
    </div>
  </template>
  ...
  const queryProduct = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/warehouses/products", {
        params: {
  ...
  const transfer = async () => {
    const toWarehouse = warehouseList.value.find(w => w.id === selectedWarehouse.value)?.name || "";
    ...
    if (res.data.successType === 3) {
      emit("new-approval", {
        ...
        display: `${localStorage.getItem("warehouse_name")}-${transferProduct.value}-${transferQty.value}-待审核`,
      });
      alert("调货申请已提交");
    }
  };
