...
def display_result(result: Dict):
    ...
    if 'relevance_score' in result:
        lines.append(f"📊 知识库匹配度: {result['relevance_score']:.1%}")

    # 语义检索结果
    semantic = result.get('semantic_results', [])
    if semantic:
        lines.append("")
        lines.append("🔍 语义相似度检索")
        lines.append("-" * 20)
        for i, semantic_result in enumerate(semantic[:3]):
            candidate = semantic_result['candidate']
            similarity = semantic_result['similarity']
            if similarity > 0.3:
                lines.append(f"相关任务{i+1}: {candidate['task']} - {candidate['text']}")
