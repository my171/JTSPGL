...
def init_db_pool(db_type: str, **kwargs) -> dict:
    ...
    if db_type == 'sqlite':
        ...
        DBPool.init_pool(db_type='sqlite', sqlite_path=path, maxconn=5)
        return {'db_type': 'sqlite', 'sqlite_path': path}

    elif db_type == 'postgres':
        DBPool.init_pool(db_type='postgres', host=kwargs['host'], port=kwargs['port'],
                         user=kwargs['user'], password=kwargs['password'], database=kwargs['database'])
        return {'db_type': 'postgres', **kwargs}

    elif db_type == 'mysql':
        DBPool.init_pool(db_type='mysql', host=kwargs['host'], port=kwargs.get('port', 3306),
                         user=kwargs['user'], password=kwargs['password'], database=kwargs['database'], maxconn=20)
        return {'db_type': 'mysql', **kwargs}
